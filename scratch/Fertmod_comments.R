...
#   # I pulled fertcounter out from the last loop because it was updating for each cell rather than each time point??
  ...
  source("./R/plot_fun1.R") # sperm
  source("./R/plot_fun2.R") # eggs
  ...
  source("./R/dispersal_func5_diff.R") # dispersal4
  ...
  (no_timestep <- 500 * sub_steps) # (s) CHANGE THIS. 125/m min ascent time @ 0.008m/s. The total exposure time = 10800 (s) or 180 (min) or 3 hrs. Needs to meet min array
  ...
  (bundlebreak <- 400 * sub_steps) # (s) 1350 From Wolstone 2004 with humulis removed. Teo  = 900 (s) = 15 (min). For 1 min iteration
  bundlebreak_sd <- 10 * sub_steps # (s)289 is mean indiv SD from Wolstone 2004 with humulis removed, see rscript
  ...
  (trans_dim <- 10 * down_scale) # (m) These are the no. of columns in teh grid
